...
do
   ffmpeg -i "$f" -i logo/lower.avi -i logo/logo.png -filter_complex \
      ...
      [bg1][2:v]overlay=10:10,drawtext=fontfile=/usr/share/fonts/truetype/BebasNeue/BebasNeue-Bold.ttf:text=$(basename "$f" | cut -f 1 -d '.'):fontcolor=white:fontsize=46::x=160:y=H-th-123:enable='between(t,0,15)" \
