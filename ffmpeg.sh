...
do
   ffmpeg -i "$f" -i logo/lower.avi -i logo/logo.png -filter_complex \
      ...
      [bg1][2:v]overlay=10:10,drawtext=fontfile=logo/Courier Prime.ttf:text=$(basename "$f" | cut -f 1 -d '.'):fontcolor=white:fontsize=36:x=318:y=h-th-130:alpha=1:enable='between(t,0,13)" \
